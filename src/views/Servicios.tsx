...
export default function Servicios() {
  const { register, handleSubmit, reset, watch, setValue, formState: { errors } } = useForm<Servicio>({
    defaultValues: {
      user: { id: '' },
      nivel: { id: 0, nombre: '' },
      seccion: { id: 0, nombre: '' },
      materia: { id: 0, nombre: '' },
      codigoCargo: '', cargo: '', puntos: 0, cantHs: 0,
      caracter: 'Interino', fechaToma: ''
    }
  })
  ...
  useEffect(() => { setValue('seccion.id', 0); setValue('materia.id', 0) }, [nivelSel])
  useEffect(() => { setValue('materia.id', 0) }, [seccionSel])

  const filteredSecciones = secciones.filter(s => String(s.nivel?.id) === String(nivelSel))
  const filteredMaterias = materias.filter(m => String(m.seccion?.id) === String(seccionSel))
  ...
  const columns = useMemo<GridColDef[]>(() => [
    { field: 'nivel', headerName: 'Nivel', flex: 1, valueGetter: (p) => p.row.nivel?.nombre || '' },
    { field: 'seccion', headerName: 'Sección', flex: 1, valueGetter: (p) => p.row.seccion?.nombre || '' },
    { field: 'materia', headerName: 'Materia', flex: 1.2, valueGetter: (p) => p.row.materia?.nombre || '' },
      ...
      <Grid item xs={12}>
        <SectionCard>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <Button variant="contained" onClick={() => setOpen(true)}>Nuevo Servicio</Button>
          </Box>
      ...
      <Dialog open={open} onClose={() => setOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent dividers>
          <Box component="form" sx={{ display: 'grid', gap: 2 }}>
            <TextField select label="Usuario" {...register('user.id', { required: 'Requerido' })}>
              {users.map(u => <MenuItem key={u.id} value={u.id}>{u.apellido}, {u.nombre}</MenuItem>)}
            </TextField>
            <TextField select label="Nivel" {...register('nivel.id', { required: 'Requerido' })}>
              {niveles.map(n => <MenuItem key={n.id} value={n.id}>{n.nombre}</MenuItem>)}
            </TextField>
            <TextField select label="Sección" {...register('seccion.id', { required: 'Requerido' })} disabled={!nivelSel}>
              {filteredSecciones.map(s => <MenuItem key={s.id} value={s.id}>{s.nombre}</MenuItem>)}
            </TextField>
            <TextField select label="Materia" {...register('materia.id', { required: 'Requerido' })} disabled={!seccionSel}>
              {filteredMaterias.map(m => <MenuItem key={m.id} value={m.id}>{m.nombre}</MenuItem>)}
            </TextField>
            <TextField label="Código Cargo" {...register('codigoCargo', { required: 'Requerido' })} />
            <TextField label="Cargo" {...register('cargo', { required: 'Requerido' })} />
            ...
            <TextField type="date" label="Fecha de Toma" InputLabelProps={{ shrink: true }} {...register('fechaToma', { required: 'Requerido' })} />
          </Box>
        </DialogContent>
        <DialogActions>
